print.ipcwcompetingrisksROC <- function(x,No.lines=5,...){
  
  # generate table to print
  
  if(x$iid==TRUE){
  tab_ou_print<-round(cbind(x$Stats,x$AUC_1*100,x$inference$vect_sd_1*100,x$AUC_2*100,x$inference$vect_sd_2*100),2)
  colnames(tab_ou_print)<-c("Cases","survivors","Other events","Censored","AUC_1 (%)","se_1","AUC_2 (%)","se_2")
  }
  else{
    tab_ou_print<-round(cbind(x$Stats,x$AUC_1*100,x$AUC_2*100),2)
