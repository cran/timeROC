print.ipcwsurvivalROC <- function(x,No.lines=5,...){
  
  
  # generate table to print
  
  if(x$iid==TRUE){
    tab_ou_print<-round(cbind(x$Stats,x$AUC*100,x$inference$vect_sd_1*100),2)
    colnames(tab_ou_print)<-c("Cases","survivors","Censored","AUC (%)","se")
  }
  else{
    tab_ou_print<-round(cbind(x$Stats,x$AUC*100),2)
