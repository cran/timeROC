...
compute_iid_decomposition_competing_risks<-function(t,n,cause,F01t,St,weights,T,delta,marker){
  ...
  ##################################################################
  # START OF STEP : Compute martingale residuals and the martingale
  # representation of the Kaplan Meier estimator of the censoring
  # survival function.
  
  
  # compute the empirical survival function corresponding to the conting process 1(\tilde{eta}=0, \tilde{T}<=t)
  hatSdeltaCensTc<-1-cumsum(Mat_data[,c("indic_Cens")])/n  

  # Build the matrix required for computing  dM_C(u) for all time u (all observed times \tilde{T}_i)
  W21<-cbind(Mat_data[,c("T","indic_Cens")],1-(1:n)/n,hatSdeltaCensTc)
  W21<-rbind(c(0,0,1,1),W21) # Add the first row corresponding to time t=0
  colnames(W21)<-c("T","indic_Cens","hatSTc","hatSdeltaCensTc")
  
  # compute hazard function of the censoring
  lambdaC<-(W21[-1,"indic_Cens"])/(n:1)  
  
  # Add the column of the hazard function of the censoring (equal to 0 at time t=0)
  W21<-cbind(W21,c(0,lambdaC))
  colnames(W21)[ncol(W21)]<-"lambdaC"
  ...
  # compute  martingale of censoring \hat{M}_{C_i}(u) for all time u (all observed times \tilde{T}_i)
  # using previous matrix
  

  # function that eats the index of the row (corresponding to the i-th observed oredered time
  # \tilde{T}_i ) and that returns \hat{M}_{C_i}(u) for all time u (all observed times \tilde{T}_i)
  CalcVecthatMC<-function(i){
    v<-WMTc2[i,]
    v[2]*as.numeric(v[1]<=WMTc2[,"T"])- c(WMTc2[0:i,"LambdaC"], rep(v[6],(n-i))) # question >= ou > en second?
  }
  
  # We obtain a matrix. Each column contains the vector of M_{C_i}(\tilde{T}_j) for  all j.
  hatMC<-sapply(1:n,CalcVecthatMC)
  ...
  # compute \int_0^t d \hat{M}_{C_l} (u)/(S_{\tilde{T}}(u))
  # for each subject l,.
  # Here time t is the time we aim to compute the iid representation of the AUC !(Remark : this step is done again just after...)
  
  # 1) select terms for the summations (integrations from 0 to t)
  dhatMCdivSTTronc<-dhatMCdivST[WMTc2[,"T"]<=t,]
  # 2) compute the sums (integrations from 0 to t)
  Int0tdMCsurEffARisk<-colSums(dhatMCdivSTTronc) 
  
  
  
  # Compute \int_0^{\tilde{T}_j} d{ \hat{M}_{C_l} (u) } / (S_{\tilde{T}}(u))
  # for each subject l, we compute for all time \tilde{T}_j.
  # l=column, j=row
  ...
  # END OF STEP : Compute martingale residuals and the martingale representation 
  # of the Kaplan Meier estimator of the censoring survival function
  ######################################################################################################
  
  
  
  ################################################
  # START OF STEP : Compute terms  {\hat{h}_{tij}}_1 and {\hat{h}_{tij}}_2
  #
  ...
  # END OF STEP : Compute terms  {\hat{h}_{tij}}_1 and {\hat{h}_{tij}}_2
  ###########################################################
 
  
  ##########################################################
  # START final step to compute iid representation of AUC(t)
  ...
  # STOP final step to compute iid representation of AUC(t)
  ########################################################## 
  ...
  sdAUC<-sd(hatIF)/sqrt(n)
  sdAUCstar<-sd(hatIFstar)/sqrt(n)
  ...
  return(list(iid_representation_AUC=hatIF,iid_representation_AUCstar=hatIFstar, sdAUC=sdAUC,sdAUCstar=sdAUCstar,
