##########################################################
# fonction autonomne, qui estime les poids en meme temps #
##########################################################
...
timeROC<-function(T,delta,marker,other_markers=NULL,cause,weighting="marginal",times,ROC=TRUE,iid=FALSE){
  ...
  if (weights$method!="marginal"){ 
    Weights_controls_1<-1/(weights$IPCW.times[,t]*n)  } else{
  ...
  # fonction to compute an area under a curve
  ...
  if (iid==TRUE){ 
    ...
    if(weighting!="marginal"){
      stop("Error : Weighting should be marginal for computing the iid representation \n Choose iid=FALSE or weighting=marginal in the arguments")
      } 
    else{
      
      # create output of iid representation for inference
      ...
      vetc_sd<-rep(NA,times=n_times)
      names(vetc_sd)<-paste("t=",times,sep="")
      
      vetc_sdstar<-rep(NA,times=n_times)    
      names(vetc_sdstar)<-paste("t=",times,sep="")   
      ...
      for (j in 1:n_times){
        ...
        #compute iid representation when AUC can be computed
          if(!is.na(AUC_1[j]) | !is.na(AUC_2[j])){
            out_iid[[j]]<-compute_iid_decomposition(t=times[j],n=n,cause=cause,F01t=CumInci[j],St=surv[j],weights,T,delta,marker)} 
          else{
            out_iid[[j]]<-NA}
        #browser()
        #save output for inference for AUC_1 when AUC_1 can be computed        
          if(!is.na(AUC_1[j])){
            ...
            vetc_sdstar[j]<-out_iid[[j]]$sdAUCstar
        ...
        #save output for inference for AUC_2 when AUC_2 can be computed         
          if(!is.na(AUC_2[j])){
            ...
            vetc_sd[j]<-out_iid[[j]]$sdAUC
            vect_iid_comp_time[j]<-out_iid[[j]]$computation_times               
          }   
      }
      inference<-list(mat_iid_rep_2=mat_iid_rep,mat_iid_rep_1=mat_iid_rep_star,
                      vect_sd_1=vetc_sdstar,vect_sd_2=vetc_sd,vect_iid_comp_time=vect_iid_comp_time)
      }
